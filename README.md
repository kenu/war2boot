...
4. **Context Path 변경사항**  
   - 기본 컨텍스트 경로는 동적으로 처리되도록 변경되었습니다.  
   - 모든 링크는 `${pageContext.request.contextPath}`를 사용하여 동적으로 생성됩니다.  
   - `application.properties`에서 `server.servlet.context-path` 값을 수정하여 컨텍스트 경로를 변경할 수 있습니다.
